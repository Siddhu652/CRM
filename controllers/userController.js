...
const updateuser = async (req, res) => {
  try {
    ...
    await finduser.update({ name, email, password, phone_number, role });
...
const loginUser = async (req, res) => {
  try {
    ...
    if (!email || !password || password == "" || email == "") {
     return res.status(400).json({ message: "no data given in body" });
    }
    ...
    return  res.status(401).json({
      ...
      const token = jwt.sign(
        { user: findUser.user_name, email: findUser.email, role: findUser.role },
